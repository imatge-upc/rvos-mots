...
from utils.utils import outs_perms_to_cpu, save_checkpoint_prev_inference_mask, load_checkpoint, get_base_params, \
    get_skip_params, merge_params
...
def trainIters(args):
    epoch_resume = 0
    model_dir = os.path.join('../models/', args.model_name + '_prev_inference_mask')
    ...
    if args.resume:
        ...
        encoder_dict, decoder_dict, _, _, load_args = load_checkpoint(args.transfer_from, args.use_gpu)
        encoder = FeatureExtractor(load_args)
    ...
    for e in range(args.max_epoch):
        ...
        if mt < (best_val_loss - args.min_delta):
            ...
            save_checkpoint_prev_inference_mask(args, encoder, decoder, enc_opt, dec_opt)
            acc_patience = 0
        else:
            acc_patience += 1
