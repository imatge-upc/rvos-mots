...
from utils.utils import outs_perms_to_cpu, save_checkpoint_prev_mask, load_checkpoint, get_base_params, get_skip_params, \
    merge_params
...
from utils.objectives import softIoULoss, focalLoss, MaskedBCELoss
import time
import math
import os
import warnings
...
def runIter(args, encoder, decoder, x, y_mask, sw_mask, resolution,
    ...
    if args.loss_penalization:
        ...
        print("SW MASK: ", sw_mask)
        print("Resolution mask: ", resolution_mask)
...
def trainIters(args):
    ...
    loaders = init_dataloaders(args, 0, 0)
    num_batches = {'train': 0, 'val': 0}
    threshold = 1
    area_range = [[0 ** 2, 1e5 ** 2], [0 ** 2, 30 ** 2], [30 ** 2, 90 ** 2], [90 ** 2, 1e5 ** 2]]
    resolution = 0
    ...
    for e in range(args.max_epoch):
        ...
        if args.multigrid:
            if e == 0:
                batch_sz = 2
                loaders = init_dataloaders(args, e, batch_sz)
            elif e == 10:
                batch_sz = 4
                loaders =init_dataloaders(args, e, batch_sz)
            ...
            if args.dataset == 'davis2017' or args.dataset == 'youtube' or args.dataset == 'kittimots':
                ...
                for batch_idx, (inputs, targets, seq_name, starting_frame) in enumerate(loaders[split]):
                    ...
                    for ii in range(max_ii):
                        ...
                        if args.linear_schedule_sampling:
